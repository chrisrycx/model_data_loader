...
GAMUT_PATH = os.getenv('GAMUT_PATH', 'gamut')
...
class GamutDataset:
    ...
    def find_files(self) -> list[str]:
        ...
        Corresponding files follow the convention: 'L_{site_abbrev}_C_...}
        :return: List of file names.
        '''
        site_files = []
        site_dir = os.path.join(GAMUT_PATH, self.site)
        for file in os.listdir(site_dir):
            if file.startswith(f'L_{site_abbrev[self.site]}_C') and file.endswith('.csv'):
    ...
    def load_csv(self, file_name: str) -> pd.DataFrame:
        ...
        '''
        file_path = os.path.join(GAMUT_PATH, self.site, file_name)
    ...
    def get_variables(self, filenames: list[str]) -> list[str]:
        '''
        Get a list of variables from the specified files.
        Variables follow the convention: 'L_{site_abbrev}_C_{variable_name}_SourceID...'
        :param filenames: List of file names to extract variables from.
        :return: List of unique variable names.
        '''
        variables = []
        site_id = f'L_{site_abbrev[self.site]}_C_'
    ...
    def get_data(self, variable: str) -> pd.Series:
        ...
        :return: DataFrame containing the data for the specified variable.
        '''
        file_name = f'L_{site_abbrev[self.site]}_C_{variable}_SourceID_1_QC_1.csv'
        file_path = os.path.join(GAMUT_PATH, self.site, file_name)
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"File {file_name} does not exist in {self.site} directory.")
